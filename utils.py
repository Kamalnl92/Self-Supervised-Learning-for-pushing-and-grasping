...
def get_obj_grasp_predictions(grasp_predictions, mask_all, num_obj):
    obj_grasp_predictions = []
    for object_idx in range(num_obj):

        obj_grasp_prediction = np.multiply(grasp_predictions, mask_all[object_idx])
        obj_grasp_prediction = obj_grasp_prediction / 255
        obj_grasp_predictions.append(obj_grasp_prediction)

    return obj_grasp_predictions
