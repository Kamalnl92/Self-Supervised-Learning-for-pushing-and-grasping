...
def main(args):
    ...
    # km 2021, there was mo check from the parser
    if args.logging_directory is None:
    ...
    robot = Robot(stage, goal_object, obj_mesh_dir, num_obj, workspace_limits,
                    ...
                    if stage == 'push_only':
                        if best_grasp_conf <= grasp_reward_threshold and nonlocal_variables['push_step'] < max_push_episode_length:
                            ...
                            if grasp_goal_conditioned or goal_conditioned:
                                ...
                                latest_goal_mask_heightmap = np.float32(latest_goal_mask_heightmap) # converting cause cv2 handels float32 and not float64
                            ...
                            if goal_conditioned:
                                ...
                                mask = robot.mask(latest_color_heightmap, goal_object)
                                mask = np.float32(mask)  # converting cause cv2 handels float32 and not float64
            ...
            if nonlocal_variables['restart_scene'] == robot.num_obj / 2: # If at end of test run, re-load original weights (before test run)
                ...
                if is_testing: # If at end of test run, re-load original weights (before test run)
                    if force_cpu:
                        ...
                        trainer.model.load_state_dict(torch.load(snapshot_file, map_location=device))
                    else:
                        trainer.model.load_state_dict(torch.load(snapshot_file))
